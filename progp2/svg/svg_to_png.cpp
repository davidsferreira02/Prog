...
namespace svg {
    ...
    ellipse *parse_circle(XMLElement *elem) {
        ...
        color fill = parse_color(elem->Attribute("fill"));
        return new ellipse(fill, {cx, cy}, {r, r});
    }
    ...
    void parse_shapes(XMLElement *elem, std::vector<shape *> &shapes, std::map<std::string, shape *> &idshapes);

    group *parse_group(XMLElement *elem, std::map<std::string, shape *> &idshapes) {
        std::vector<shape *> shapes;
        parse_shapes(elem, shapes, idshapes);
        return new group(shapes);
    }

    shape *parse_use(XMLElement *elem, std::map<std::string, shape *> &idshapes) {
       shape *Nova_shape;
        auto iterador=idshapes.begin();
        std::string href = elem->Attribute("href");
       for(iterador;iterador!=idshapes.end();iterador++){
           if(iterador->first==href.substr(1,href.size())){
               Nova_shape=(iterador->second)->duplicate();
           }
       }
        return Nova_shape;
    }
    ...
    void parse_shapes(XMLElement *elem, std::vector<shape *> &shapes, std::map<std::string, shape *> &idshapes) {
        for (auto child_elem = elem->FirstChildElement();
            ...
            if (type == "ellipse") {
                ...
                s = parse_group(child_elem, idshapes);
            } else if (type == "use") {
                s = parse_use(child_elem, idshapes);
            } else {
                std::cout << "Unrecognized shape type: " << type << std::endl;
                continue;
            }
            if (child_elem->Attribute("id")) {
                idshapes.insert({child_elem->Attribute("id"), s});
            }
    ...
    void svg_to_png(const std::string &svg_file, const std::string &png_file) {
        ...
        std::vector<shape *> shapes;
        std::map<std::string, shape *> idshapes;
        parse_shapes(elem, shapes, idshapes);
        int width = elem->IntAttribute("width");
        int height = elem->IntAttribute("height");
